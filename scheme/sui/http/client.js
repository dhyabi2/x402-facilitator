// Browser-only Sui wallet helper for generic x402 resource servers.
...
export async function prepareX402Payment(options = {}) {
    ...
    emitPaymentEvent(options, 'payment.prepare', 'Preparing stablecoin transaction', { method, path });
    ...
    if (prepared.prepareTransaction?.transaction) {
        if (!wallet.executeTransaction) {
            throw new Error('Selected wallet cannot execute the stablecoin prepare transaction');
        }
        ...
        if (prepareStatus && prepareStatus !== 'success') {
            throw new Error(prepareResult.effects?.status?.error || 'stablecoin prepare transaction failed');
        }
        ...
        if (prepared.prepareTransaction?.transaction) {
            throw new Error('Prepared stablecoin balance is not indexed yet');
        }
    }
